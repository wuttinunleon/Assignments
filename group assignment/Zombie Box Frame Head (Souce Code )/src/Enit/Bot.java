...
public class Bot extends Status {
    Player player;
    BufferedImage skin;
    ...
    public Bot(GameEngine gameEngine,float x , float y,BufferedImage skin){
        super(gameEngine,x,y,Status.DEFAULT_BOT_WIDTH,Status.DEFAULT_BOT_HEIGHT,Assets.monster);
        bound.x=width/3;
        bound.y=0;
        bound.width=width/2;
        bound.height=height;
        ...
        this.skin=skin;
        randomPlayer();
        animationDown = new Animation(180, Assets.zombie_down);
        animationNow = new Animation(180, Assets.zombie_down);
        animationUp = new Animation(180, Assets.zombie_up);
        animationRight = new Animation(180, Assets.zombie_right);
        animationLeft = new Animation(180, Assets.zombie_left);
                ...
                for (int i = 0; i < gameEngine.getMap().getEnitityManager().getEnitities().size(); i++) {
                    if(gameEngine.getMap().getEnitityManager().getEnitities().get(i).toString().equals(bothit)){
                        if (bothit.equals("Player1")){
                            gameEngine.getMap().getEnitityManager().getEnitities().get(i).health--;
                        }
